...
> Array&lt;Team&gt; crupdateTeams(userId, companyId, crupdateTeam)
...
> deleteTeamById(userId, companyId, id)
...
> Team getTeamById(userId, companyId, id)
...
> PaginatedResponse getTeams(userId, companyId, page, pageSize, jobId)
...
async function example() {
  ...
  const body = {
    ...
    // number (optional)
    pageSize: 20,
    // string | Filter teams by job ID (optional)
    jobId: job_001,
...
| **jobId** | `string` | Filter teams by job ID | [Optional] [Defaults to `undefined`] |
